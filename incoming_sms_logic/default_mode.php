       ...
       // if($user_profile->profile_status!='complete')
        ...
        if(isset($message[1]))
        {
            ...
            for($x=1;$x<count($message);$x++)
            {
                ...
                $message_part.=$message[$x];
            }
            $nickname = trim($message[0]);
            ...
            $recipient_data=db_layer::execute($query,"GET Profile By Nickname");

            if(mysqli_num_rows($recipient_data)==0)
            {
            ...
            if($nickname!=null)
            {
                ...
                $amount_data = mysqli_fetch_assoc($amount_sql_data);
				...
				$query="INSERT INTO SORMConversationLog (src_adress,dst_adress,transaction_id,datetime_receive, receiver_nick)"                          
                        ."VALUES (".$source_adress.",".$recipient_data['msisdn'].",".$transaction_id.",NOW(),'".$nickname."');";
        ...
        if($debug_mode)
        {
            echo('Wrong Request passed all checks ,no catch'.PHP_EOL);
        }
        exit;
    }
